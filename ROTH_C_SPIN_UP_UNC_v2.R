...
WD_FOLDER <- ("H:/BisonRothC/OUTPUTS")

# Vector must be an empty points vector. 
setwd(WD_FOLDER)
Vector<-vect("H:/BisonRothC/INPUTS/TARGET_POINTS/target_points.shp")

# Stack_Set_1 is a stack that contains the spatial variables 
Stack_Set_1<- rast(c("H:/BisonRothC/INPUTS/STACK/Stack_Set_SPIN_UP_AOI1.tif"))
...
for (i in 1:dim(Vector_variables)[1]) {
  ...
  if (any(is.na(Evp[,2])) | any(is.na(Temp[,2])) | any(is.na(SOC_im[i])) | any(is.na(clay_im[i])) | any(is.na(Precip[,2]))  |  any(is.na(Cov2[,2]))  |  any(is.na(Cov1[,1]))  | any(is.na(DR_im[i])) |  (SOC_im[i]<0) | (clay_im[i]<0) ) {C_INPUT_EQ[i,2]<-0}
  else{
...
setwd("H:/BisonRothC/OUTPUTS/1_SPIN_UP")
writeVector(C_INPUT_EQ, filename= "SPIN_UP_AOI1.shp") 
plot(C_INPUT_EQ)
