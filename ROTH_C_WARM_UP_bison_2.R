...
#Open empty vector
Vector<-vect(c("H:/BisonRothC/INPUTS/TARGET_POINTS/target_points.shp"))
#Open Warm Up Stack
Stack_Set_warmup<- rast(c("INPUTS/STACK/Stack_Set_WARM_UP_AOI1.tif"))
# Open Result from SPIN UP PROCESS. A vector with 5 columns , one for each pool
Spin_up<-vect(c("H:/BisonRothC/OUTPUTS/1_SPIN_UP/SPIN_UP_AOI1.shp"))
...
PREC<-rast(c("H:/BisonRothC/INPUTS/TERRA_CLIMATE_1/Prec_Stack_01-19_TC.tif"))
TEMP<-rast(c("H:/BisonRothC/INPUTS/TERRA_CLIMATE_1/Temp_Stack_01-19_TC.tif"))*0.1
PET<-rast(c("H:/BisonRothC/INPUTS/TERRA_CLIMATE_1/PET_Stack_01-19_TC.tif"))*0.1
...
LU_AOI<-rast(c("H:/BisonRothC/INPUTS/LAND_USE/ESA_Land_Cover_12clases_FAO_AOI.tif"))
...
Vector_points<-extract(Stack_Set_warmup,Vector, bind=TRUE)
Vector_points<-extract(TEMP,Vector_points, bind=TRUE)
Vector_points<-extract(PREC,Vector_points,bind=TRUE)
Vector_points<-extract(PET,Vector_points,bind=TRUE)
Vector_points<-extract(NPP,Vector_points,bind=TRUE)
Vector_points<-extract(NPP_MEAN_MIN,Vector_points,bind=TRUE)
Vector_points<-extract(NPP_MEAN_MAX,Vector_points,bind=TRUE)
WARM_UP<-Vector
...
nppBand<-length(Stack_Set_warmup)+clim_layers*3+2

firstClimLayer<-length(Stack_Set_warmup)+2
...
for (i in 1:(length(Vector_points))) {
  ...
  for (w in 1:(dim(TEMP)[3]/12)) {
    ...
    if (any(is.na(Evp[,2])) | any(is.na(Temp[,2])) | any(is.na(SOC_im[i, ])) | any(is.na(clay_im[i, ])) | any(is.na(Spin_up[i,3])) | any(is.na(NPP_im[i, ])) | any(is.na(Precip[,2])) | any(is.na(Cov2[,2])) |  any(is.na(Cov1[,1])) | any(is.na(DR_im[,1])) | any(SOC_im[i, ]<0) | any(clay_im[i, ]<0) | (Spin_up[i,3]<=0) | is.na(Spin_up[i,3])) {WARM_UP[i,2]<-0}
    else{
      ...
      DR<-DR_im[w,1]              # DPM/RPM (decomplosable vs resistant plant material.) Arkan: this is default
      ...
      if (w==1) {Cinputs<-(as.numeric(Spin_up[i,3])/NPP_81_00)*NPP_M[w]} else {print("oh no!1"); Cinputs[w]<-(Cinputs[[w-1]]/ NPP_M[w-1]) * NPP_M[w]} 
      
      # Cinputs MIN
      
      Tmin<-mean(Temp[,2]*1.02)
      Pmin<-sum(Precip[,2]*0.95)
      NPP_M_MIN<-NPPmodel(Pmin,Tmin,"miami")*(1/100)*0.5
      NPP_M_MIN<-(LU_im[i, ]==2 | LU_im[i, ]==12 | LU_im[i, ]==13)*NPP_M_MIN[w]*0.53+ (LU_im[i, ]==4)*NPP_M_MIN[w]*0.88 + (LU_im[i, ]==3 | LU_im[i, ]==5 | LU_im[i, ]==6 | LU_im[i, ]==8)*NPP_M_MIN[w]*0.72
      
      if (w==1) {Cinputs_min<-(Spin_up[i,10]/NPP_81_00)*NPP_M_MIN[w]} else {print("oh no!2"); Cinputs_min[w]<-(Cinputs_min[[w-1]]/ NPP_M_MIN[w-1]) * NPP_M_MIN[w]} 
      ...
      if (w==1) {Cinputs_max<-(Spin_up[i,11]/NPP_81_00)*NPP_M_MAX[w]} else {Cinputs_max[w]<-(Cinputs_max[[w-1]]/ NPP_M_MAX[w-1]) * NPP_M_MAX[w]} 
      ...
      if (w ==1) {
        b_R <- 0.41 # bison waste estimated decomposability (bison ratio)
        
        B_c_mean <- (0.0687/NPP_M[w]) # % of NPP that goes through bison (bison contribution)
        DR_mean <- B_c_mean*b_R + DR*(1-B_c_mean) # o.o687 t ha-1 year-1, 0.5 DPM/RPM ratio
      ...
      if (w==1) {
        f_wp<-Roth_C(Cinputs=Cinputs[1],years=years,DPMptf=Spin_up[i,5], RPMptf=Spin_up[i,6], BIOptf=Spin_up[i,7], HUMptf=Spin_up[i,8], FallIOM=Spin_up[i,9],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR_mean,bare1=bare1,LU=LU_im[i, ])
      } else {
      ...
      #f_wp_t<- c(-0.2736413, 25.9119905, 1.5248120, 66.5064903, 9.6248886)
      
      # Run the model for minimum values
      if (w==1) {
        f_wp_min<-Roth_C(Cinputs=Cinputs_min, years=years,DPMptf=Spin_up[i,13], RPMptf=Spin_up[i,14], BIOptf=Spin_up[i,15], HUMptf=Spin_up[i,16], FallIOM=Spin_up[i,17],Temp=Temp*1.02,Precip=Precip*0.95,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC*0.8,clay=clay*0.9,DR=DR_min,bare1=bare1,LU=LU_im[i, ])
      } else {
      ...
      if (w==1) {
        f_wp_max<-Roth_C(Cinputs=Cinputs_max[1],years=years,DPMptf=Spin_up[i,19], RPMptf=Spin_up[i,20], BIOptf=Spin_up[i,21], HUMptf=Spin_up[i,22], FallIOM=Spin_up[i,23],Temp=Temp*0.98,Precip=Precip*1.05,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC*1.2,clay=clay*1.1,DR=DR_max,bare1=bare1,LU=LU_im[i, ])
      } else {
...
names(WARM_UP)[2]="SOC_FAO"
names(WARM_UP)[3]="Cin_t0"
names(WARM_UP)[4]="SOC_t0"
names(WARM_UP)[5]="DPM_w_up"
names(WARM_UP)[6]="RPM_w_up"
names(WARM_UP)[7]="BIO_w_up"
names(WARM_UP)[8]="HUM_w_up"
names(WARM_UP)[9]="IOM_w_up"
names(WARM_UP)[10]="Cin_mean"
names(WARM_UP)[11]="SOC_t0min"
names(WARM_UP)[12]="DPM_w_min"
names(WARM_UP)[13]="RPM_w_min"
names(WARM_UP)[14]="BIO_w_min"
names(WARM_UP)[15]="HUM_w_min"
names(WARM_UP)[16]="IOM_w_min"
names(WARM_UP)[17]="SOC_t0max"
names(WARM_UP)[18]="DPM_w_max"
names(WARM_UP)[19]="RPM_w_max"
names(WARM_UP)[20]="BIO_w_max"
names(WARM_UP)[21]="HUM_w_max"
names(WARM_UP)[22]="IOM_w_max"
names(WARM_UP)[23]="Cin_min"
names(WARM_UP)[24]="Cin_max"


# SAVE the Points (shapefile)
setwd("H:/BisonRothC/OUTPUTS/2_WARM_UP")
terra::writeVector(WARM_UP, "WARM_UP_AOI_final", filetype="ESRI Shapefile", overwrite=TRUE)
